...
#  - Deterministic analyzers with trust scoring & audit trail
...
def modified_z_scores(x: np.ndarray) -> np.ndarray:
    """Robust modified z-scores (|z| >= 3.5 => outlier)."""
...
def trust_score_base(procs_df: pd.DataFrame) -> Tuple[float, Dict[str, float], pd.Series]:
    ...
    score = float(np.exp(np.mean(np.log(np.maximum(components, EPS)))))  # geometric mean, epsilon-safe
...
class VendorOutliersAnalyzer(Analyzer):
    ...
    def run(self, q: str, pbs: pd.DataFrame, procs: pd.DataFrame, ctx: ChatContext) -> ChatAnswer:
        ...
        code_steps.append("Compute modified z-scores; select |z| >= 3.5")
        ...
        summary = (
            f"**Vendor payment outliers** detected via robust modified z‑score (|z| ≥ 3.5). "
...
class ProcurementRedFlagsAnalyzer(Analyzer):
    ...
    def run(self, q: str, pbs: pd.DataFrame, procs: pd.DataFrame, ctx: ChatContext) -> ChatAnswer:
        ...
        grp = (
            df.groupby(["day", "vendor_id", "category"])
            .agg(count=("amount", "count"), total=("amount", "sum"), max_amt=("amount", "max"))
            .reset_index()
        )
        grp["threshold"] = grp["category"].map(thresholds).fillna(10_000.0)

        split_flags = grp[
            (grp["count"] >= 3) & (grp["total"] > grp["threshold"]) & (grp["max_amt"] < grp["threshold"])
        ].copy()
        ...
        cat_daily = (
            df.groupby(["category", df["date"].dt.to_period("D")])["amount"]
            .sum()
            .rename("amount")
            .to_timestamp()
        )
        ...
        cat_df["rolling_median"] = cat_df.groupby("category")["amount"].transform(
            lambda s: s.rolling(90, min_periods=30).median()
        )
...
def run_chat(query: str, pbs: pd.DataFrame, procs: pd.DataFrame, ctx: ChatContext) -> ChatAnswer:
    q = (query or "").strip()
    if not q:
    ...
    analyzer = matched[0][1]
    ans = analyzer.run(q, pbs=pbs, procs=procs, ctx=ctx)
...
def sidebar_scope(pbs: pd.DataFrame, key_prefix: str = "scope_") -> Tuple[pd.DataFrame, ChatContext]:
    ...
    with st.sidebar.expander("Red‑flag thresholds (split‑purchase)"):
        ...
        for k in sorted(set(list(DEFAULT_SPLIT_PURCHASE_THRESHOLDS.keys()) + scope["expense_type"].dropna().unique().tolist())):
            current[k] = float(
...
def simulator_tab(pbs_scope: pd.DataFrame, procs: pd.DataFrame) -> None:
    ...
    cat_overrides = {
        c: st.sidebar.slider(f"{c} change (%)", -50, 150, 0, step=5, key=f"sim_cat_{c}") for c in categories
    }
...
def chat_tab(pbs_scope: pd.DataFrame, procs: pd.DataFrame, ctx: ChatContext) -> None:
    st.subheader("Chat (Vetted, Retrieval‑Only)")
    st.caption(
        "Answers are computed from the loaded datasets with rule‑based analyzers, "
        "evidence tables, and trust scoring. If a question is out of scope or evidence is weak, the system will say so."
    )
    ...
    if user_q:
        ...
        ans = run_chat(user_q, pbs=pbs_scope, procs=procs, ctx=ctx)
...
def main() -> None:
    ...
    st.title("📊 PBS What‑if + Vetted Chat (Retrieval‑Only, No Hallucinations)")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("This app is retrieval‑only (no free‑text LLM). Outliers use modified z‑score (|z| ≥ 3.5).")
